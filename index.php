...
class Rcl_Gateway_Freekassa extends Rcl_Gateway_Core {
	...
	function get_form( $data ) {
		...
		$shop_id	 = rcl_get_commerce_option( 'fk_shop' );
		$secret_1	 = rcl_get_commerce_option( 'fk_secret_1' );

		$amount = number_format( $data->pay_summ, 2, '.', '' );
	...
	function result( $process ) {
		...
		$my_signature = md5( implode( ':', array(
			rcl_get_commerce_option( 'fk_shop' ),
			$_REQUEST["AMOUNT"],
			rcl_get_commerce_option( 'fk_secret_2' ),
